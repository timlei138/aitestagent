...
launch_app，收到 COOLDOWN_SKIP 后改调 terminate_run），真调 agents.llm_runtime._run_agent，
对比「守卫启用(LIMIT=3)」与「守卫禁用(LIMIT=1e9)」两种结果。

若启用时 launch 真实执行仅 ~3 次、出现 COOLDOWN_SKIP、且最终 terminate_run，
而禁用时 launch 耗满 max_turns、无 SKIP、无 terminate_run —— 则证明守卫逻辑正确，
...
class FakeDevice:
    ...
    def current_app(self, refresh=False):
        ...
        return {"package": "com.android.launcher", "activity": f"Launcher_{self._n}"}
...
class FakeChat:
    ...
    def invoke(self, messages):
        cnt = sum(
            1
            for m in messages
            if hasattr(m, "content") and "COOLDOWN_SKIP: launch_app" in str(m.content)
        )
...
if __name__ == "__main__":
    ...
    if on["launch_real"] <= 3 and on["skip_seen"] > 0 and on["reported"]:
        print("  [OK] 守卫逻辑正确：冗余 launch 被拦截并逼出 terminate_run")
