...
### 1.1 进化方向：给 Agent 必要的代码兜底，让 LLM 发挥更大作用

Agent 进化的方向**不是**「用代码替 LLM 做判断」，而是**用代码搭好稳固的地基（契约 / 事实 / 基础设施），让 LLM 在其上放开手脚做它真正擅长的事**（理解页面、找元素、决策下一步、语义判断）。本迁移计划删的正是"代码替 LLM 做判断"的那 ~800 行；而保留/新增的，只应是给 LLM 的稳定地基。

**边界怎么划——「判断 vs 事实/契约」：**

| 类型 | 例子 | 归属 |
|---|---|---|
| **感知 / 决策 / 语义判断**（有歧义、要理解） | 该点哪个元素、下一步做什么、"购物车是否显示了刚加的商品" | **交给 LLM**（本计划正把代码从这里下线：`_score_element`/`_rank_click_candidates`/fallback 等） |
| **确定性事实**（可被程序核实） | 任务是否已调用 report_done、页面**字面**是否含"已支付"、元素是否存在 | 代码给 **ground truth**，别让 LLM 靠"看一眼"来猜 |
| **工具契约 / 运行基础设施**（有限且稳定） | click 统一返回 OK/NOT_FOUND/AMBIGUOUS（§10.4）、结束只能经 report_done、循环检测/冷却、感知稳定等待、`check_dangerous`、pixel 级执行 | **代码兜底**（这是给 LLM 的地基，不是替它做判断） |

**判断某段兜底代码是否健康的试金石——「契约收敛，补丁发散」：**

- ✅ **契约 / 不变量（可以加）**：数量**有限、稳定**，不随场景增长。例：「结束只能通过 report_done」「工具统一结构化返回码」「可判定事实由工具给 ground truth」「感知前等页面稳定」。这类兜底让 LLM 的输出有可靠落点，反而**放大**了 LLM 的作用。
- ⚠️ **特例补丁（要警惕）**：针对某个具体异常写的 if/else，会**无限增长**（"如果在设置页反复点应用列表就……""如果输出里出现某字样就……"）。补丁越堆越多，是 approach 有问题的信号——正确解法通常是**换更强的模型 / 改 prompt / 改工具设计**，而不是再加一个补丁。本计划下线的 legacy 兜底（fallback 回退循环、known-rid/pct-bounds 猜测）正是发散型补丁的典型。

> 一句话：**代码负责"稳"（契约、事实、基础设施），LLM 负责"活"（理解、决策、判断）。** 凡是让"稳"的部分收敛、让 LLM 更放得开的兜底，就做；凡是让代码陷入无限特例补丁的，就停下来换思路。这也解释了"模型能力影响大"的健康解法：优先**更强模型 + 更干净契约**，而非更多防御性代码。
